...
let is_line_safe line =
  ...
  let rec loop last_number rest ascending_type =
    match rest with
    | [] -> true
    | first :: rest -> (
        ...
        match ascending_type with
        | Some value ->
            if Option.get ordering <> value then false
            else if is_diff_incorrect (Option.get last_number) first then false
            else loop (Some first) rest ascending_type
        | None -> loop (Some first) rest ascending_type)
  in
  loop None numbers None
